...
<script>
	basket.require({ url: '<?php echo get_template_directory_uri(); ?>/dist/Diaspora.css', unique: 10,  execute: false })
	.then(function(responses) {
		...
		.then(function() {
			...
			.then(function() {
        		basket.require({ url: '<?php echo get_template_directory_uri(); ?>/dist/Diaspora.js', unique: 10 })
                .then(function() {
                    if (!window.$ || !window.DP) {
                        localStorage.clear()
                    }
